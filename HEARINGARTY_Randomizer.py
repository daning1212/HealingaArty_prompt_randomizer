...
class HEARINGARTY_PromptRandomizerV10:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
            ...
            "optional": {
                "헤어스타일": (cls.get_options([
                    ...
                    "medium hair, side swept", "short hair, bob cut", "short hair, pixie cut", "short hair, boyish",
                    "short hair, undercut", "short hair, bowl cut", "very short hair, buzz cut", "shoulder length hair, straight",
                    ...
                    "twin tails, high", "twin tails, low", "twin tails, side", "braided ponytail, single",
                    "braided twin tails, double", "bun, high", "bun, low", "bun, messy", "bun, sleek",
                    "bun, space buns", "bun, donut bun", "bun, ballerina bun", "half up half down, ponytail",
                    "half up half down, bun", "half up half down, braid", "braid, single", "braid, french braid",
                    "braid, dutch braid", "braid, fishtail braid", "braid, crown braid", "braid, side braid",
                    "braid, milkmaid braid", "braid, waterfall braid", "braid, rope braid", "braid, boxer braids",
                    ...
                    "hair down, sleek", "hair down, tousled", "side swept hair, glamorous", "hair flipped over shoulder, sexy",
                    "hair tucked behind ear, cute", "hair covering one eye, mysterious", "hair blowing in wind, dynamic",
                    "hair in face, sensual", "hair accessories, ribbon", "hair accessories, headband",
                    "hair accessories, hair clip", "hair accessories, flower crown", "black hair, natural",
                    "brown hair, brunette", "blonde hair, golden", "blonde hair, platinum", "red hair, auburn",
                ...
                "란제리": (cls.get_options([
                    "lingerie, lace bra and thong set, red", "lingerie, lace bra and panty set, black",
                ...
                "수영복": (cls.get_options([
                    "bikini, basic bikini", "bikini, triangle bikini", "bikini, bandeau bikini",
                ...
                "세로포즈": (cls.get_options([
                    "standing facing forward, vertical", "one hand on hip standing, vertical",
                    ...
                    "sitting legs crossed, vertical", "kneeling sitting, vertical", "sitting cross-legged, vertical",
                    "sitting leaning on desk, vertical", "squatting looking at camera, vertical", "squatting head down, vertical",
                    "sitting on stairs legs together, vertical", "kneeling on floor, vertical", "sitting one knee up, vertical",
                    ...
                    "standing both hands in pockets, vertical", "standing hand covering mouth, vertical", "stretching pose, vertical",
                    ...
                    "holding book standing, vertical", "kneeling on bed looking back, sexy vertical", "bending over desk, sexy vertical",
                    "sitting with legs spread, sexy vertical", "squatting with knees apart, sexy vertical", "leaning forward showing cleavage, sexy vertical",
                    "sitting backwards on chair, sexy vertical", "standing with hip pop, sexy vertical", "leaning against wall seductively, sexy vertical",
                    "sitting on floor legs open, sexy vertical", "bending to tie shoes revealing, sexy vertical", "straddling chair, sexy vertical",
                    "kneeling with back arched, sexy vertical", "standing hand on thigh, sexy vertical", "sitting on edge of bed legs crossed, sexy vertical",
                    "stretching arms up revealing midriff, sexy vertical", "standing back to camera looking over shoulder, sexy vertical",
                    "kneeling between legs, sexy vertical", "sitting on knees leaning back, sexy vertical"
                ],),

                "가로포즈": (cls.get_options([
                    "none", "random", "lying on bed front, horizontal", "lying face down on bed, horizontal",
                    "lying on side head on arm, horizontal", "lying on stomach legs raised, horizontal", "lying spread eagle on bed, horizontal",
                    ...
                    "prone position looking up, sexy horizontal"
                ],),
            }
        }
...
RETURN_TYPES = ("STRING", "STRING", "INT")
    ...
    def generate(self, 시드, 랜덤_모드, **kwargs):
        # 1. 시드 결정 로직: 순차적 모드일 때 시드값 계산
        if 랜덤_모드 == "순차적":
            실제_시드 = (시드 if 시드 != -1 else 0) + 1
        ...
        for key, val in kwargs.items():
            ...
            if val not in [None, "none"]:
                # get_options()로 인해 리스트 앞 3개는 ["none", "random", "sequential"]임
                # 따라서 실제 데이터는 [3:]부터 시작합니다.
                전체_옵션 = self.INPUT_TYPES()["optional"][key][0]
                실제_옵션리스트 = 전체_옵션[3:]
                
                if val == "random":
                    picked = rng.choice(실제_옵션리스트)
                elif val == "sequential":
                    # 시드값을 리스트 길이로 나눈 나머지값을 인덱스로 사용 (순환)
                    picked = 실제_옵션리스트[실제_시드 % len(실제_옵션리스트)]
                else:
                    picked = val
    ...
    def IS_CHANGED(cls, 시드, 랜덤_모드, **kwargs):
        # 완전랜덤이거나 순차적 모드일 때는 값이 항상 변하도록 설정
        if 랜덤_모드 in ["완전랜덤", "순차적"] or 시드 == -1:
            return time.time()
        return 시드
NODE_CLASS_MAPPINGS = {"HEARINGARTY_PromptRandomizerV10": HEARINGARTY_PromptRandomizerV10}
NODE_DISPLAY_NAME_MAPPINGS = {"HEARINGARTY_PromptRandomizerV10": "HEARINGARTY Prompt Randomizer V10"}
